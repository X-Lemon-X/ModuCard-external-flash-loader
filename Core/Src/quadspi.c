...
/* USER CODE BEGIN 0 */
#define HAL_QPSI_CHIP_ERASE_TIMEOUT_MAX_VALUE  200000U
...
uint8_t CSP_QSPI_Erase_Chip(void)
{
	...
	if (QSPI_AutoPollingMemReady_ChipErase() != HAL_OK)
	{
		return HAL_ERROR;
	}
...
uint8_t QSPI_AutoPollingMemReady_ChipErase(void)
{
	...
	if (HAL_QSPI_AutoPolling(&hqspi, &sCommand, &sConfig, HAL_QPSI_CHIP_ERASE_TIMEOUT_MAX_VALUE) != HAL_OK)
	{
		return HAL_ERROR;
	}
...
uint8_t QSPI_Configuration(void)
{
	...
	MODIFY_REG(test_buffer, 0x02, 0x02);
...
uint8_t CSP_QSPI_EraseSector(uint32_t EraseStartAddress, uint32_t EraseEndAddress)
{
	...
	{
		sCommand.Address = (EraseStartAddress & 0x0FFFFFF);
